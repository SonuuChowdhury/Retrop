...
interface TaxBreakdownEntry { name: string; percent: number; amount: number; inclusive: boolean }
interface TaxData {
  totalRevenue: number;
  totalTaxes: number;
...
type TabKey = 'sales' | 'orders' | 'dishes' | 'payments' | 'customers' | 'taxes';
            ...
            <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} contentContainerStyle={modalSt.scroll}>
              ...
              {(invoice.items ?? []).map((item: any, i: number) => (
                <View key={i} style={modalSt.itemRow}>
                  ...
                  <Text numberOfLines={1} style={[modalSt.itemTotal, { color: colors.text }]}>₹{item.total.toFixed(2)}</Text>
                </View>
              ))}
              ...
              <View style={modalSt.totalsBox}>
                <View style={modalSt.totalsRow}>
                  ...
                  <Text numberOfLines={1} style={{ color: colors.text }}>₹{invoice.subtotal.toFixed(2)}</Text>
                </View>
                ...
                {(invoice.discountBreakdown ?? []).map((disc: any, i: number) => (
                  <View key={i} style={modalSt.totalsRow}>
                    ...
                    <Text numberOfLines={1} style={{ color: '#2a9d5c', fontWeight: '700' }}>-₹{(Number(disc.amount) || 0).toFixed(2)}</Text>
                  </View>
                ))}
                ...
                {invoice.discountAmount > 0 && (invoice.discountBreakdown ?? []).length === 0 && (
                  <View style={modalSt.totalsRow}>
                    ...
                    <Text numberOfLines={1} style={{ color: '#2a9d5c', fontWeight: '700' }}>-₹{invoice.discountAmount.toFixed(2)}</Text>
                  </View>
                )}
                ...
                {(invoice.order.taxBreakdown ?? []).map((tax: any, i: number) => (
                  <View key={i} style={modalSt.totalsRow}>
                    ...
                    <Text numberOfLines={1} style={{ color: colors.textSecondary }}>₹{(Number(tax.amount) || 0).toFixed(2)}</Text>
                  </View>
                ))}
                ...
                <View style={[modalSt.totalsRow, { borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 10, marginTop: 6 }]}>
                  ...
                  <Text numberOfLines={1} style={{ fontSize: 18, fontWeight: '800', color: colors.primary }}>₹{invoice.grandTotal.toFixed(2)}</Text>
                </View>
              </View>
...
const KpiCard = memo(({ icon, label, value, sub, accent, delay, colors }: {
  ...
  <Animated.View entering={FadeInDown.delay(delay).duration(380)} style={[kpiSt.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
    ...
    <Text numberOfLines={1} adjustsFontSizeToFit style={[kpiSt.value, { color: colors.text }]}>{value}</Text>
        ...
        {entries.map(([date, val]) => {
          ...
          return (
            <View key={date} style={chartSt.barCol}>
              <Text numberOfLines={1} adjustsFontSizeToFit style={[chartSt.barVal, { color: colors.textSecondary }]}>{val > 0 ? fmt(val) : ''}</Text>
...
const DishRow = memo(({ dish, rank, colors, delay }: { dish: DishEntry; rank: number; colors: any; delay: number }) => {
  ...
  return (
    <Animated.View entering={FadeInDown.delay(delay).duration(340)} style={[dishSt.row, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      <Text numberOfLines={1} style={[dishSt.price, { color: colors.text }]}>₹{dish.price}</Text>
    </Animated.View>
  );
});
      ...
      <Animated.View entering={FadeInDown.delay(delay).duration(340)} style={[aOrderSt.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <View style={aOrderSt.top}>
          ...
          <View style={aOrderSt.topRight}>
            <Text numberOfLines={1} style={[aOrderSt.amount, { color: colors.text }]}>₹{order.finalAmount ?? order.totalAmount}</Text>
...
const TaxesTab = memo(({ data, colors }: { data?: TaxData; colors: any }) => {
  if (!data) return <EmptyState message="Tax data unavailable" icon="percent" colors={colors} />;
  return (
    <>
      <SectionHeader title="Tax Summary (Past 7 Days)" colors={colors} delay={0} />
      <View style={{ flexDirection: 'row', gap: 10, marginBottom: 12 }}>
        <KpiCard icon="cash-multiple" label="Revenue"   value={fmt(data.totalRevenue)} accent={colors.primary} delay={60} colors={colors} />
        <KpiCard icon="percent"       label="Taxes"     value={fmt(data.totalTaxes)}   accent={colors.success} delay={120} colors={colors} />
      </View>
      <View style={{ flexDirection: 'row', gap: 10, marginBottom: 12 }}>
        <KpiCard icon="tag-outline"   label="Discounts" value={fmt(data.totalDiscounts)} accent={colors.warning} delay={180} colors={colors} />
        <KpiCard icon="calculator"    label="Subtotal"  value={fmt(data.totalSubtotal)}  accent="#7C4DFF"        delay={240} colors={colors} />
      </View>

      <SectionHeader title="Tax Breakdown" colors={colors} delay={300} />
      {data.taxBreakdown.length === 0 ? (
        <EmptyState message="No tax breakdown available" icon="percent" colors={colors} />
      ) : (
        <View style={[taxTabSt.breakdownContainer, { backgroundColor: colors.card, borderColor: colors.border }]}>
          {data.taxBreakdown.map((t, idx) => (
            <View key={t.name + idx} style={[taxTabSt.row, idx > 0 && { borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 10, marginTop: 10 }]}>
              <View>
                <Text style={[taxTabSt.taxName, { color: colors.text }]}>{t.name}</Text>
                <Text style={[taxTabSt.taxPercent, { color: colors.textSecondary }]}>
                  {t.percent}% {t.inclusive ? 'Inclusive' : 'Exclusive'}
                </Text>
              </View>
              <Text numberOfLines={1} style={[taxTabSt.taxAmount, { color: colors.primary }]}>₹{t.amount.toFixed(2)}</Text>
            </View>
          ))}
        </View>
      )}
...
const taxTabSt = StyleSheet.create({
  breakdownContainer: { borderRadius: 16, borderWidth: 1.5, padding: 16, marginBottom: 20 },
  row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  taxName: { fontSize: 15, fontWeight: '700' },
  taxPercent: { fontSize: 12, marginTop: 2 },
  taxAmount: { fontSize: 16, fontWeight: '800' }
});
...
export default function AnalyticsScreen() {
  ...
  const activeTabContent = useMemo(() => {
    ...
    switch (activeTab) {
      ...
      case 'taxes':     return <TaxesTab data={data.taxes} colors={c} />;
      default: return null;
    }
  }, [activeTab, data, c]);
