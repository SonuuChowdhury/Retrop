      ...
      <nav className="status-bar__steps" aria-label="Order progress">
        {STEPS.map((step, idx) => {
          ...
          return (
            <div
              key={step.id}
              className={`status-bar__step ${isActive ? 'status-bar__step--active' : ''} ${isDone ? 'status-bar__step--done' : ''}`}
            ...
            >
              ...
              {idx < STEPS.length - 1 && (
                <div className={`status-bar__connector ${isDone ? 'status-bar__connector--done' : ''}`} />
              )}
            </div>
          );
        })}
