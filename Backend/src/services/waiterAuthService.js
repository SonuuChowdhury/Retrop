...
export const waiterAuthService = {
  ...
  generateAccessToken: (waiterId) => {
    return jwt.sign({ waiterId, role: 'waiter' }, WAITER_JWT_SECRET, { expiresIn: ACCESS_EXPIRY });
  },

  // ── Generate waiter refresh token ─────────────────────────────────────
  generateRefreshToken: (waiterId) => {
    return jwt.sign({ waiterId }, WAITER_REFRESH_SECRET, { expiresIn: REFRESH_EXPIRY });
  },
  ...
  login: async (mobile, password, fcmToken, ipAddress, userAgent) => {
    try {
      ...
      // 4. Generate tokens
      const accessToken = waiterAuthService.generateAccessToken(waiter.waiterId);
      const refreshToken = waiterAuthService.generateRefreshToken(waiter.waiterId);
      ...
      await redis.set(REDIS_KEYS.waiterSession(waiter.waiterId), sessionData, ACCESS_EXPIRY_SEC);
  ...
  refreshAccessToken: async (refreshToken) => {
    try {
      ...
      const sessionData = await redis.get(REDIS_KEYS.waiterSession(decoded.waiterId));
      ...
      if (!waiter || !waiter.isActive) {
        await redis.del(REDIS_KEYS.waiterSession(decoded.waiterId));
        return {
          success: false,
      ...
      const newAccessToken = waiterAuthService.generateAccessToken(waiter.waiterId);

      // Extend Redis TTL
      await redis.set(REDIS_KEYS.waiterSession(waiter.waiterId), {
        ...sessionData,
  ...
  // ── Logout ────────────────────────────────────────────────────────────
  logout: async (waiterId) => {
    try {
      await redis.del(REDIS_KEYS.waiterSession(waiterId));
      ...
      logger.info(`Waiter logged out: ${waiterId}`);
      return { success: true };
    } catch (err) {
