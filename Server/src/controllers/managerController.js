...
export const getComprehensiveAnalytics = async (req, res) => {
  try {
    ...
    if (requestedMetrics.includes('taxes')) {
      promises.push(
        orderAnalyticsService.getTaxAnalytics(days).then((result) => {
          if (result.success) {
            analyticsData.taxes = result.data;
          } else {
            errors.push(`Tax analytics: ${result.error}`);
          }
        })
      );
    }
