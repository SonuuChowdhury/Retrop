...
export const orderAnalyticsService = {
  ...
  getTodaySales: async () => {
    try {
      ...
      const { data: orders, error } = await supabase
        .from('orders')
        .select('ordersId, totalAmount, finalAmount, orderStatus, createdAt')
  ...
  getWeeklySales: async () => {
    try {
      ...
      const { data: orders, error } = await supabase
        .from('orders')
        .select('totalAmount, finalAmount, createdAt, isPaymentCompleted')
      ...
      Object.keys(dailySales).forEach((k) => {
        dailySales[k] = parseFloat(dailySales[k].toFixed(2));
      });
  ...
  getSalesByPaymentMethod: async (days = 7) => {
    try {
      ...
      const { data: orders, error } = await supabase
        .from('orders')
        .select('totalAmount, finalAmount, paymentMethod')
        .gte('createdAt', startDate)
        .eq('isPaymentCompleted', true);
      ...
      Object.keys(paymentMethods).forEach((k) => {
        paymentMethods[k] = parseFloat(paymentMethods[k].toFixed(2));
      });
  ...
  // Get tax, discount and revenue analytics
  getTaxAnalytics: async (days = 7) => {
    try {
      const startDate = daysAgoStartIST(days);

      const { data: orders, error } = await supabase
        .from('orders')
        .select('totalAmount, finalAmount, discountAmount, gstAmount, taxBreakdown, createdAt')
        .gte('createdAt', startDate)
        .eq('isPaymentCompleted', true);

      if (error) {
        logger.error('Failed to fetch tax analytics', error.message);
        return { success: false, error: 'Failed to fetch tax analytics data' };
      }
      ...
      orders.forEach((order) => {
        const rev = order.finalAmount !== null ? parseFloat(order.finalAmount) : parseFloat(order.totalAmount || 0);
        totalRevenue += rev;
        totalTaxes += parseFloat(order.gstAmount) || 0;
        totalDiscounts += parseFloat(order.discountAmount) || 0;
        totalSubtotal += parseFloat(order.totalAmount) || 0;

        if (Array.isArray(order.taxBreakdown)) {
          order.taxBreakdown.forEach((tax) => {
            if (tax && tax.name) {
              const name = tax.name;
              if (!taxBreakdownSummary[name]) {
                taxBreakdownSummary[name] = {
                  name,
              ...
              taxBreakdownSummary[name].amount += parseFloat(tax.amount) || 0;
            }
          });
        }
      });
      ...
      const taxSummaryList = Object.values(taxBreakdownSummary).map(t => ({
        ...t,
      ...
      return {
        ...
        data: {
          ...
          totalDiscounts,
          totalSubtotal,
          taxBreakdown: taxSummaryList,
          ordersCount: orders.length
        }
      };
    } catch (error) {
      logger.error('Tax analytics fetch error', error.message);
      throw error;
    }
  },
};