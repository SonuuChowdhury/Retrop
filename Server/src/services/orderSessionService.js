...
export const orderSessionService = {
  ...
  submitCustomerInfo: async (tableId, sessionToken, customerName, customerMobile) => {
    try {
      ...
      // Get restaurantId from session, fallback to DB if missing
      ...
      if (customerError) {
        logger.error('Failed to upsert customer:', customerError.message, customerError);
        return { success: false, error: 'Failed to save customer details.', code: 500 };
      }
